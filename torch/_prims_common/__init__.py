...
def _resolve_rng_module():
    # Prefer the active accelerator (CUDA, XPU, etc.); fall back to CUDA for
    # back-compat with environments that have not opted into torch.accelerator.
    acc = (
        torch.accelerator.current_accelerator()
        if hasattr(torch, "accelerator")
        else None
    )
    dev_type = acc.type if acc is not None else "cuda"
    mod = getattr(torch, dev_type, None)
    if mod is None or not mod.is_available():
        raise RuntimeError(f"{dev_type} not available")
    return mod, dev_type
