...
Requires: torchao, CUDA or MPS or XPU
"""
...
HAS_XPU = torch.xpu.is_available() if hasattr(torch, "xpu") else False
HAS_GPU = HAS_CUDA or HAS_MPS or HAS_XPU
DEVICE = "cuda" if HAS_CUDA else "mps" if HAS_MPS else "xpu" if HAS_XPU else "cpu"


@unittest.skipIf(not HAS_GPU, "requires CUDA, MPS, or XPU")
...
class Int4MMFusionTests(InductorTestCase):
    ...
    def setUpClass(cls):
        ...
        if HAS_XPU:
            ...
            cls.packing_format = Int4PackingFormat.PLAIN
        else:
            from torchao.quantization.quantize_.workflows.int4.int4_packing_format import (
                Int4PackingFormat,
            )
            ...
            cls.Int4PackingFormat = Int4PackingFormat
            cls.packing_format = Int4PackingFormat.TILE_PACKED_TO_4D
    ...
    def test_int4_mm_fusion_three_way(self):
        ...
        int4_config = self.Int4WeightOnlyConfig(
            group_size=128,
            int4_packing_format=self.packing_format,
        )
